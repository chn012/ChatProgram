...
void user::deleteChatHistory(string name)
{
	...
	if (!curr) {
		cout << "\nYou don't have any chat history with this friend!" << endl;
	}
	else
	{
...
//major issue printing message is not mutual 
...
/**
void user::sendMSG(string recipient, string text) {
		msg* message = new msg(usrname, recipient, text);
		msgList->add(msgList->count,message);
		recipient->getMsgList()->add(recipient->getMsgList()->count(),message);
}
**/
